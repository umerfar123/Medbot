...
# Set up your Telegram bot token and username
TOKEN = 'YOUR_TOKEN'
...
# Set up API endpoint and key for the USDA Food Composition Databases
url = 'https://api.nal.usda.gov/fdc/v1/foods/search'
api_key = 'YOUR_USDA_API_KEY'  # Replace with your USDA API key
...
# Define data array for image processing
...
def recom(condition, message):
    ...
    for i in range(len(column1_values)):
        if condition in column1_values[i]:
            ...
            amazon_url = f"https://www.amazon.in/s?k=product for {condition}"
            link_text = 'Check out this website!'
            message_text = f'<a href="{amazon_url}">{link_text}</a>'
            bot.send_message(message.chat.id, text=message_text, parse_mode='HTML')        
            bot.send_photo(message.chat.id, photo=amazon_url)
...
def handle_callback(call):
    if call.data == 'submit':
        ...
        cnx = mysql.connector.connect(user='root', password='YOUR_DB_PASSWORD', host='127.0.0.1', database='pythondatas')
        cursor = cnx.cursor()
